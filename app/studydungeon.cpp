...
{
public:
    MainMenuScene(ConsoleUI::UIManager &uiManager, std::function<void()> openFibonacciScene) : m_uiManager(uiManager)
    {
...
int main()
{
    ...
    fibonacciScene = std::make_shared<FibonacciScene>(uiManager, [&]() { uiManager.setCurrentScene(mainMenuScene); });

    // Create MainMenuScene
    mainMenuScene = std::make_shared<MainMenuScene>(uiManager, [&]() { uiManager.setCurrentScene(fibonacciScene); });
