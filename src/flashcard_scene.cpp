...
// Helper function to draw wrapped text
void EditDecksScene::drawWrappedText(std::shared_ptr<ConsoleUI::ConsoleWindow> window,
                                     const std::string &text,
                                     int x,
                                     int y,
                                     int width)
...
void EditFlashcardScene::render(std::shared_ptr<ConsoleUI::ConsoleWindow> window)
{
    ...
    if (m_currentCardIndex < m_deck.cards.size())
    {
        ...
        std::string progress =
            "Card " + std::to_string(m_currentCardIndex + 1) + " of " + std::to_string(m_deck.cards.size());
